...
const verifyToken = async (req, res, next) => {
  try {
    ...
    const user = await User.findOne({
      _id: new mongoose.Types.ObjectId(decoded.user_id),
    });
...
// REPLACE the existing verifyAdminToken with this:
const verifyAdminToken = async (req, res, next) => {
  try {
    ...
    // generateAdminToken uses { id: ... }
    // generateToken uses { user_id: ... }
...
const verifyCoordinateToken = async (req, res, next) => {
  try {
    ...
    return res
      .status(500)
      .json({ detail: "Co-Ordinator token Verification Failed." });
  }
};
...
const isPlumber = (req, res, next) => {
  if (req.user.role !== "PLUMBER") {
    return res
      .status(403)
      .json({ detail: "Access denied. Plumber role required." });
  }
  next();
};
...
const isAdmin = (req, res, next) => {
  if (req.user.role !== "ADMIN") {
    return res
      .status(403)
      .json({ detail: "Access denied. Admin role required." });
  }
  next();
};
